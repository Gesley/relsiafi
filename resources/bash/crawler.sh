...
# declare -r keypath='/mnt/srv/RELSIAFI/resources/keys/'
# declare -r keypath='/mnt/srv/ctis/relsiafi/resources/keys/'
declare -r keypath='/dados/www/html/relsiafi/resources/keys/'
...
if [ -z "$1" ] && [ -z "$2" ]; then
    ...
    declare -r cpf=$(echo "$1" | base64 --decode | openssl rsautl -decrypt -inkey "$keypath"private-key.pem)
    declare -r password=$(echo "$2" | base64 --decode | openssl rsautl -decrypt -inkey "$keypath"private-key.pem)
        ...
        #tesseract $temp_dir/prepared_captcha.tif $temp_dir/result_tesseract -s
            ...
            declare -r captcha_cookie=$(awk 'FNR==6 { print $7}' $temp_dir/cookie_captcha.txt | sed 's/randomica=//')
                ...
                declare -r  login_response=$(curl -X POST $uri_post \
                    -k \
                    --silent \
                    -d "QueryString='hidMensagemErroTransacao=&er=1&ctx=0&txtCPF=$cpf&txtSenha=$password&txtTextoImagem=$captcha_cookie&cmbTransacao=Download" \
                ...
                if grep -q 'SEW_menu.asp?pstrTransacao=Download' <<< $login_response; then
                    ...
                    if grep -q 'index.asp?msg=4' <<< $login_response; then
                        echo 'Credenciais incorretas'
                    fi

                    if grep -q 'index.asp?msg=5' <<< $login_response; then
                        echo 'Verificação do CAPTCHA falhou!'
                    fi
                fi
